...
let GameController = class GameController {
    ...
    async updateGame(id, update) {
        ...
        if (update.id !== undefined) {
            throw new routing_controllers_1.BadRequestError('id cannot be updated');
        }
        if (update.color !== undefined) {
            if (!colors.includes(update.color)) {
                throw new routing_controllers_1.BadRequestError(`colors must have one those values ${colors}`);
            }
        }
        if (update.board !== undefined) {
            if (moves(game.board, update.board) > 1) {
                throw new routing_controllers_1.BadRequestError(` no more than one move per turn ${update} `);
            }
        }
